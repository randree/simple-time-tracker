...
class MainWindow(Gtk.Window):
    ...
    def update_total_time(self):
        ...
        for elapsed_time, in times:
            ...
            hours, mins, secs = [
                int(part) for part in time_parts[:-1]] + [float(time_parts[-1])]
    ...
    def on_start_button_clicked(self, widget):
        if not self.running:
            ...
            cursor.execute("INSERT INTO records (category_id, elapsed_time, timestamp) VALUES (?, ?, ?)",
                           (category_id, formatted_time, current_timestamp))
            self.conn.commit()
            ...
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE settings SET active_category_id = ?", (active_category_id,))
            self.conn.commit()
